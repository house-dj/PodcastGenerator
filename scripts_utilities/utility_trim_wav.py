...
def trim_wav_seconds(input_file, trim_count=1,
                     # output_file,
                     start_time=None, end_time=None, duration=None):
    """
    Trims a WAV file using time values in seconds or 'M:S' format.
    ...
    the same input_file.
    :param output_file: Path for the trimmed output file - WAV or MP3 depending on code.
...
clipslist =     [["253.5", "285.3"]
                 ]

print("Number of clips to extract: " + str(len(clipslist)))
audio_chunks=[]
output_file = "Generate Audio/voices/clips for generation/Dan_Wang.wav"
for i in range(0, len(clipslist)):
    trim_clip = trim_wav_seconds(
        "Generate Audio/voices/Google Books Narrators/Male Canadian-Chinese_Dan_Wang.wav",
        trim_count=i+1,
