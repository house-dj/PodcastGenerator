...
CONFIG = {
    ...
    "csv": str(Path("./Generate Text Scripts/reference files/topics_Chinese.csv").resolve()),
    # Output directory for generated files
    "outdir": str(Path("./Generate Text Scripts/generated_text_files").resolve()),
    # Model: for free tier limiter in your script, you likely set gemini-2.5-flash
    "model": "gemini-2.5-flash",
    # Prompts & schema hint (your existing files)
    # "report_prose_sample": str(Path("./Generate Text Scripts/reference files/JB_Bury_sample.txt"))
    "script_instructions": str(Path("./Generate Text Scripts/reference files/LLM Script Generation Instructions_Gemini Optimised_Chinese.txt").resolve()),
    "outline_schema_hint": str(Path("./Generate Text Scripts/reference files/Outline JSON Chinese.json").resolve()),
...
from generate_reports_and_scripts_async_chinese import run_all as pipeline_run_all  # type: ignore
from generate_reports_and_scripts_async_chinese import main as pipeline_main       # type: ignore
...
async def run_via_run_all() -> None:
    ...
    await pipeline_run_all(
        ...
        model=CONFIG["model"],
        script_instructions_path=CONFIG["script_instructions"],
        outline_schema_hint_path=CONFIG["outline_schema_hint"],
...
def run_via_main() -> None:
    ...
    """
    argv = [
        "generate_reports_and_scripts_async_chinese.py",
        "--csv", CONFIG["csv"],
        "--outdir", CONFIG["outdir"],
        #"--report_prose_sample", CONFIG["report_prose_sample"],
        "--script_instructions", CONFIG["script_instructions"],
        "--outline_schema_hint", CONFIG["outline_schema_hint"],
